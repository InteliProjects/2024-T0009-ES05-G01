...
export default function AtualizaONGForm() {
  ...
  useEffect(() => {
    api
      .get(`/ongs/${ongId}`)
      .then((response) => {
        setOngInfo(response.data.ong);
      })
      .catch((error) => {
        console.error('Erro ao obter informações da ONG:', error);
      });
  }, [ongId]);

  const handleFinalStep = (formData) => {
    console.log('Dados do formulário para enviar:', formData);
    console.log('ID da ONG:', ongId);
    delete formData.id;
  ...
  return (
    <section className="container mx-auto min-h-[70vh]">
      <h1 className="text-3xl font-bold mb-4">Atualização de ONG</h1>
      <p className="mb-8 text-gray-600">
        Preencha os campos abaixo para atualizar a sua ONG na plataforma.
      </p>
      {ongInfo && (
        <CustomForm
          etapas={2}
